...
public interface IMongoRepository<T> where T : class
{
    ...
    public Task<List<DadosHistorico>> GetHistoricoPesquisa(int pageNumber, int pageSize, string usuarioFilter, string cnpjFilter);
    public Task<Resposta<T>> GetPesquisa(string id);
    Task<long> GetTotalBatchCountAsync();
    Task<long> GetTotalCountAsync( string usuarioFilter, string cnpjFilter);
}